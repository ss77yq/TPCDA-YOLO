...
class SepConv2d(torch.nn.Module):
    def __init__(self,
        ...
        # 深度卷积层，groups=in_channels表示每个输入通道有一个卷积核
...
class WindowAttention(nn.Module):
    def __init__(self, dim, win_size, num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0.,
        ...
        # 根据token_projection 的值，选择性地创建了投影层，可以是卷积投影或线性投影
...
def window_partition(x, win_size, dilation_rate=1):
    ...
    if dilation_rate != 1:
        ...
        # 得到 windows 张量，其形状为 [B', Wh, Ww, C]，Wh 和 Ww 表示划分后的窗口高度和宽度
...
class LeWinTransformerBlock(nn.Module):
    def __init__(self, dim, shift_size=0,
        ...
        self.dim = dim
        # 图像尺寸为256
        self.input_resolution = (256 / 4, 256 / 4)
        ...
        if cross_modulator:
            self.cross_modulator = nn.Embedding(self.win_size * self.win_size, dim)  # cross_modulator
            self.cross_attn = Attention(dim, self.num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                                        proj_drop=drop,
        ...
        self.attn = WindowAttention(
            dim, win_size=to_2tuple(self.win_size), num_heads=self.num_heads,
...
if __name__ == '__main__':
    # 创建 LeWinTransformerBlock 实例
    block = LeWinTransformerBlock(dim=16*16)
    ...
    print(f"输入数据的形状为：{x.shape}")

    # 进行前向传播
    output = block(x.view(B, H*W, C))

    print(f"经过LeWinTransformer模块的输出打印为：{output.shape}")  # 打印输出的形状