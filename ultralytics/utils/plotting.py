...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
        ...
        self.lw = line_width or max(round(sum(im.size if input_is_pil else im.shape) / 3 * 0.003), 2)
        if self.pil:  # use PIL
...
def plot_results(file="path/to/results.csv", dir="", segment=False, pose=False, classify=False, on_plot=None):
    ...
    # 检测指标修改
        ...
        fig, ax = plt.subplots(2, 8, figsize=(21, 6), tight_layout=True)
        index = [1, 2, 3, 8, 9, 11, 12, 13, 4, 5, 6, 7, 10, 14, 15, 16]
    ax = ax.ravel()
...
# 修改：可视化
def feature_visualization_img(x):
    """
    Visualize feature maps of a given model module during inference.
    ...
    """
    _, channels, height, width = x.shape  # batch, channels, height, width
    save_dir = Path("E:/task_yolo/FA-YOLO/syq/detect_visual")
    save_dir.mkdir(parents=True, exist_ok=True)  # create directory if it does not exist
    f = save_dir / f"features_img{len(list(save_dir.glob('features_img*.png'))) + 1}.png"  # filename

    blocks = torch.chunk(x[0].cpu(), channels, dim=0)  # select batch index 0, block by channels
    n = 1
    _, ax = plt.subplots(1, 1, tight_layout=True)  # 8 rows x n/8 cols
...
# 修改：可视化
def feature_visualization_pix(x):
    """
    Visualize feature maps of a given model module during inference.
    ...
    f = save_dir / f"features_pix{len(list(save_dir.glob('features_pix*.png'))) + 1}.png"  # filename
